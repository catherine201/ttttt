  ...
  return class DraggableArea extends React.Component {
    ...
    componentDidMount() {
      ...
      this.props.getAddTagFunc &&
        this.props.getAddTagFunc(this.addTag.bind(this));
    }
    ...
    componentWillReceiveProps({ tags }) {
      ...
      if (
        ...
        tags.some((tag, i) => tag.id !== this.state.tags.get(i).id)
      ) {
    ...
    componentDidUpdate(prevProps, { tags }) {
      this.tagChanged =
        this.tagChanged ||
        this.state.tags.some(
          (tag, i) => !tags.get(i) || tag.id !== tags.get(i).id
        );
    }
    ...
    setTags(tags, callback) {
      this.setState({ tags }, () => {
        ...
        this.state.tags.forEach(t => {
          ...
          //   console.log(draggableTag.firstChild);
    ...
    dragElement(elmnt, id, parent) {
      ...
      const elementDrag = e => {
        ...
        const movedX = clientX - prevX;
        const movedY = clientY - prevY;
        prevX = clientX;
        prevY = clientY;
        const t = elmnt.offsetTop + movedY;
        const l = elmnt.offsetLeft + movedX;
        elmnt.style.top = `${t}px`;
        elmnt.style.left = `${l}px`;

        const baseCenterTop = parent.offsetTop + elmnt.offsetHeight / 2;
        const baseCenterLeft = parent.offsetLeft + elmnt.offsetWidth / 2;
        // The center position of the tag
        const ctop = baseCenterTop + t;
        const cleft = baseCenterLeft + l;
        ...
        for (i = 0; i < this.positions.length - 1; i++) {
          ...
          if (
            ...
            if (
              ...
              const cur = this.state.tags.get(index);
      ...
      const closeDragElement = () => {
        ...
        const eRect = elmnt.getBoundingClientRect();
        const x = eRect.left + eRect.width / 2;
        const y = eRect.top + eRect.height / 2;
        if (
          x < rect.left ||
          x > rect.right ||
          y < rect.top ||
      ...
      const dragStart = e => {
        ...
        while (
          window.parentDragTag &&
          !window.parentDragTag.classList.contains('DraggableTags-tag-drag')
        ) {
